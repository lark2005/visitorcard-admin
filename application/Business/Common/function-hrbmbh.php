...
function sendMsg_post($mobile,$content) {
	...
	// 发送参数
	$param = array('userid'=>"410408");
	$password = "164489";
	// yyyyMMddHHmmss
	$timespan = date("YmdHis",time());
	$param['timespan'] = $timespan;
	// y原始密码+时间 mD5 大写
	//先得到密码的密文
	$md5Password = md5($password."".$timespan);
	//再把密文中的英文母全部转为大写
	$md5Password = strtoupper($md5Password);
	$param['pwd'] = $md5Password;
	...
	// 发送短信
	// echo "执行发送短信 url=".$url."   ,param=".$param;
